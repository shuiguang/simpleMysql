...
class DB
{
    ...
    public function connect($db_config = array())
    {
        ...
        if(is_null($this->link) || !empty($db_config))
        {
            ...
            {
                $this->link = $func($config['host'].':'.$config['socket'], $config['user'], $config['pwd']);
            }else{
                $this->link = $func($config['host'].':'.$config['port'], $config['user'], $config['pwd']);
            }
            if(!$this->link)
            {
    ...
    public function query($sql, $method='', $data=array())
    {
        ...
        {
            $sql_error = mysql_errno($this->link);
            if(!empty($sql_error))
            {
                //服务端断开时带参数强行重连一次
        ...
        switch($method)
        {
            ...
            case 'insert':                          //插入数据 返回最后插入的ID
                if($this->auto == 'yes')
                {
                    $returnv = mysql_insert_id($this->link);
                }else{
                    $returnv = $result;
                }
                break;
            case 'delete':
            case 'update':                            //update 
                $returnv = mysql_affected_rows($this->link);
                break;
            default:
                $returnv = $result;
        }
