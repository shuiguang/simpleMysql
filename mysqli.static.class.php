...
class DB
{
    ...
    public function connect($db_config = array())
    {
        ...
        if(is_null(self::$link) || !empty($db_config))
        {
            ...
            {
                self::$link = new mysqli($config['host'], $config['user'], $config['pwd'], $config['database'], $config['port'], $config['socket']);
            }else{
                self::$link = new mysqli($config['host'], $config['user'], $config['pwd'], $config['database'], $config['port']);
            }
            if(!self::$link)
            {
            ...
            self::$config = $config;
            self::$link->query('SET NAMES '.$config['charset']);
            //检测缓存目录
    ...
    public function insert($array = null, $filter = 1)
    {
        ...
        foreach($field_arr as $field)
        {
            $fields .= self::$table.'.'.$field.',';
        }
        $fields = rtrim($fields, ',');
        $sql = 'INSERT INTO '.self::$table.'('.$fields.') VALUES ('.implode(',', array_fill(0, count($array), '?')).')';
    ...
    public function update($array = null, $filter = 1)
    {
        ...
        if(is_array($array))
        {
            ...
            {
                $s .= self::$table.'.'.$this->_add_special_char($k).'=?,';
                $data[] = $v;
            }
            $s = rtrim($s, ',');
            $setfield = $s;
    ...
    public function query($sql, $method='', $data=array())
    {
        ...
        $this->connect();
        try{
            $stmt = self::$link->prepare($memkey);     //准备好一个语句
        }catch(Exception $e)
        {
            //服务端断开时带参数强行重连一次
            if(self::$link->errno == 2006 || self::$link->errno == 2013)
            {
                //必须关闭之后才能重新连接成功
                self::$link->close();
                $this->connect(self::$config);
                $stmt = @self::$link->prepare($memkey);
                if(self::$link->errno)
                {
        ...
        if(count($value) > 0)
        {
            ...
            if($stmt)
            {
                call_user_func_array(array($stmt, 'bind_param'), $this->_refValues($value));
            }
        }
        $returnv = $result = null;
        if($stmt)
        {
            ...
            switch($method)
            {
                case 'select':                          //查所有满足条件的
                    $stmt->store_result(); 
                    $returnv = $this->_getAll($stmt);
                    break;
                case 'find':                            //只要一条记录的
                    $stmt->store_result(); 
                    if($stmt->num_rows > 0) {
                        $returnv = $this->_getOne($stmt);
                    }else{
                        $returnv = false;
                    }
                    break;
                case 'total':                           //返回总记录数
                    $stmt->store_result(); 
                    $row = $this->_getOne($stmt);
                    $returnv = $row['count'];
                    break;
                case 'insert':                          //插入数据 返回最后插入的ID
                    if(self::$auto == 'yes')
                    {
                        $returnv = $stmt->insert_id;
                    }else{
                        $returnv = $result;
                    }
                    break;
    ...
    public static function _escape_string_array($array)
    {
        ...
        foreach($array as $val)
        {
            ...
            //加斜杠
            $value[] = self::$link->real_escape_string($val);
        }
        return $value;
    }
