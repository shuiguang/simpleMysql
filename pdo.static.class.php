...
class DB
{
    ...
    public function connect($db_config = array())
    {
        ...
        if(is_null(self::$link) || !empty($db_config))
        {
            ...
            self::$link = new PDO($dsn, $config['user'], $config['pwd'], array(PDO::ATTR_PERSISTENT => $config['pconnect']));
            if(!self::$link)
            {
    ...
    public function __call($func, $args)
    {
        ...
        }else if($func === 'table')
        {
            ...
            if(isset(self::$fields_cache[self::$options['table']]))
            {
                ...
                {
                    //这里sql语句不可能出错,不获取fields信息也能组成sql语句,没必要重新连接数据库
                    return $this;
                }
    ...
    public function query($sql, $method='', $data=array())
    {
        ...
        try{
            ...
            //这一句可能会抛出异常到页面,self::$link->errorInfo()无法捕捉到异常
        ...
        switch($method)
        {
            ...
            case 'insert':                          //插入数据 返回最后插入的ID
                if(self::$auto == 'yes')
                {
                    $returnv = self::$link->lastInsertId();
                }else{
                    $returnv = $result;
                }
                break;
    ...
    {
        self::$link->setAttribute(PDO::ATTR_AUTOCOMMIT, 0);
    ...
    {
        self::$link->commit();
        self::$link->setAttribute(PDO::ATTR_AUTOCOMMIT, 1);
    }
