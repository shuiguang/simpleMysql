...
class DB
{
    ...
    public function connect($db_config = array())
    {
        ...
        if(is_null($this->link) || !empty($db_config))
        {
            ...
            //连接数据库查询
            $this->link = new mysqli($config['host'], $config['user'], $config['pwd'], $config['database'], $config['port']);
            if(isset($config['socket']) && $config['socket'])
            {
                $this->link = new mysqli($config['host'], $config['user'], $config['pwd'], $config['database'], $config['port'], $config['socket']);
            }else{
                $this->link = new mysqli($config['host'], $config['user'], $config['pwd'], $config['database'], $config['port']);
            }
            if(!$this->link)
    ...
    public function query($sql, $method='', $data=array())
    {
        ...
        $this->sql = $sql;
        try{
            $stmt = $this->link->prepare($memkey);     //准备好一个语句
        }catch(Exception $e)
        {
            //服务端断开时带参数强行重连一次
            if($this->link->errno == 2006 || $this->link->errno == 2013)
            {
                //必须关闭之后才能重新连接成功
                $this->link->close();
    ...
    public function _escape_string_array($array)
    {
        ...
        foreach($array as $val)
        {
            ...
            //加斜杠
            $value[] = $this->link->real_escape_string($val);
        }
        return $value;
    }
